...
class MessageHandleRelationshipMigration {
  ...
  static void _migrateMessageHandleRelationships() {
    try {
      ...
      Logger.info("Found ${allHandles.length} handles. Processing messages in batches...", 
          tag: "DB-Migration");
      ...
      Logger.error("Failed to migrate message-handle relationships!", 
          error: e, trace: stack, tag: "DB-Migration");
      rethrow;
    }
  }
  ...
  static void verify() {
    try {
      ...
      final messagesWithRelations = Database.messages
          .query(Message_.handleRelation.notNull())
          .build()
          .count();
      
      final messagesFromMe = Database.messages
          .query(Message_.isFromMe.equals(true))
          .build()
          .count();
      ...
      final double completionRate = expectedWithRelations > 0 
          ? messagesWithRelations / expectedWithRelations 
          : 1.0;
      ...
      if (completionRate < 0.95) {
        Logger.error(
            ...
            tag: "DB-Migration");
        throw Exception("Handle relationship verification failed - completion rate: ${(completionRate * 100).toStringAsFixed(1)}%");
      }
      ...
      Logger.error("Failed to verify handle relationships!", 
          error: e, trace: stack, tag: "DB-Migration");
      rethrow;
    }
  }
