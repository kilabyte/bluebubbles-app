...
// ============================================================================
// NEW: BlueBubbles Design System Context Extensions
// ============================================================================

/// Unified color access - shorter and more semantic than context.theme.colorScheme.*
extension BBColors on BuildContext {
  ...
  Color get properSurface => _colorScheme.properSurface;
  Color get properOnSurface => _colorScheme.properOnSurface;
  ...
  Color messageBubble(bool iMessage) => _colorScheme.bubble(this, iMessage);
  Color onMessageBubble(bool iMessage) => _colorScheme.onBubble(this, iMessage);
  Color get iMessageBubble => _colorScheme.iMessageBubble;
  Color get oniMessageBubble => _colorScheme.oniMessageBubble;
  Color get smsBubble => _colorScheme.smsBubble;
  Color get onSmsBubble => _colorScheme.onSmsBubble;
}

/// Semantic text styles - shorter than context.theme.textTheme.X.copyWith(...)
extension BBTextStyles on BuildContext {
  ...
  // Bubble text (from extension)
  TextStyle get bubbleText => (theme.extensions[BubbleText] as BubbleText?)?.bubbleText ?? bodyMedium;
}

/// Skin/theme helper shortcuts - shorter than SettingsSvc.settings.skin.value == Skins.*
extension BBSkinHelpers on BuildContext {
  Skins get currentSkin => SettingsSvc.settings.skin.value;
  
  bool get iOS => currentSkin == Skins.iOS;
  bool get isMaterial => currentSkin == Skins.Material;
  bool get samsung => currentSkin == Skins.Samsung;
  
  // Tablet mode
  bool get showAltLayout =>
      SettingsSvc.settings.tabletMode.value &&
      (!isPhone || width / height > 0.8) &&
      width > 600 &&
      !LifecycleSvc.isBubble;
}

/// Dark mode and color adjustment helpers
extension BBThemeMode on BuildContext {
  bool get isDark => ThemeSvc.inDarkMode(this);
  bool get isLight => !isDark;
  Brightness get brightness => theme.colorScheme.brightness;
  
  // Color adjustment with theme awareness
  Color lightenOrDarken(Color color, [double percent = 10]) {
    return color.themeLightenOrDarken(this, percent);
  }
  
  Color darken(Color color, [double percent = 10]) {
    return color.darkenPercent(percent);
  }
  
  Color lighten(Color color, [double percent = 10]) {
    return color.lightenPercent(percent);
  }
}

/// Window effects opacity helpers - standardizes opacity logic
extension BBWindowEffects on BuildContext {
  bool get hasWindowEffect => SettingsSvc.settings.windowEffect.value != WindowEffect.disabled;
  WindowEffect get windowEffect => SettingsSvc.settings.windowEffect.value;
  
  /// Get opacity level for current window effect and theme
  double getOpacity(OpacityLevel level) {
    if (!hasWindowEffect) return OpacityLevel.full.value;
    if (!WindowEffects.dependsOnColor()) return OpacityLevel.none.value;
    
    // Use custom opacity if set
    final customOpacity = isDark 
        ? SettingsSvc.settings.windowEffectCustomOpacityDark.value
        : SettingsSvc.settings.windowEffectCustomOpacityLight.value;
    
    // If custom opacity is set, scale the level by the custom value
    return level.value * customOpacity;
  }
  
  /// Apply theme-aware opacity to a color based on window effects
  Color applyWindowOpacity(Color color, OpacityLevel level) {
    return color.withValues(alpha: getOpacity(level));
  }
}

/// Standardized color alpha/opacity API using new Flutter API
extension BBAlpha on Color {
  /// Use new withValues API for consistency
  Color alpha(double opacity) {
    assert(opacity >= 0.0 && opacity <= 1.0);
    return withValues(alpha: opacity);
  }
  
  /// Apply opacity level enum
  Color opacity(OpacityLevel level) {
    return withValues(alpha: level.value);
  }
}

/// Design token access via BuildContext
extension BBDesignSystemExtension on BuildContext {
  Skins get _currentSkin => SettingsSvc.settings.skin.value;
  
  BBSpacing get spacing => BBSpacing();
  BBRadiusSet get radius => BBRadiusSet(_currentSkin);
  BBSizing get sizing => BBSizing();
  BBDuration get duration => BBDuration();
}

/// Radius helper set for current theme
class BBRadiusSet {
  final Skins skin;
  
  const BBRadiusSet(this.skin);
  
  double get small => BBRadius.small(skin);
  double get medium => BBRadius.medium(skin);
  double get large => BBRadius.large(skin);
  double get extraLarge => BBRadius.extraLarge(skin);
  
  BorderRadius get smallBR => BBRadius.smallBR(skin);
  BorderRadius get mediumBR => BBRadius.mediumBR(skin);
  BorderRadius get largeBR => BBRadius.largeBR(skin);
  BorderRadius get extraLargeBR => BBRadius.extraLargeBR(skin);
}
